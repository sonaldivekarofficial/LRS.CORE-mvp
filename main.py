...
\f0\fs24 \cf0 # main.py - Latent Recursion System v5.0 FULL MVP (Complete & Final)\
# Everything is in this single file: 80 validated questions, 8 recursions, biome, scoring, \
# direct visible link between recursions and the 6 elements, enhanced result display\
...
\
# =============================================\
# FULL 80 NEUTRAL QUESTIONS FROM VALIDATED SCALES\
# =============================================\
questions = [\
    # Vitality (1\'9610)\
...
\
    # Meaning (51\'9660)\
    \{"id": 51, "element": "Meaning", "text": "I understand my life's meaning.", "options": ["Absolutely untrue", "Mostly untrue", "Somewhat untrue", "Can\'92t say", "Somewhat true", "Mostly true", "Absolutely true"]\},\
    \{"id": 52, "element": "Meaning", "text": "My life has a clear sense of purpose.", "options": ["Absolutely untrue", "Mostly untrue", "Somewhat untrue", "Can\'92t say", "Somewhat true", "Mostly true", "Absolutely true"]\},\
...
\
    ...
    \{"id": 68, "element": "Recursion", "text": "People in my family called me \'93stupid\'94 or \'93useless\'94.", "options": ["Never true", "Rarely true", "Sometimes true", "Often true", "Very often true"]\},\
    ...
    \{"id": 76, "element": "Recursion", "text": "I didn\'92t have enough to eat.", "options": ["Never true", "Rarely true", "Sometimes true", "Often true", "Very often true"]\},\
...
element_groups = \{\
    ...
    "Recursion": questions[60:80]\
\}\
\
# 8 Possible Recursions \'96 Fully Defined\
recursions_defs = \{\
    "abandonment_fear": \{"title": "Abandonment Fear", "desc": "Recurring fear of being left that leads to isolation and self-sabotage in relationships and opportunities."\},\
    "perfectionism_loop": \{"title": "Perfectionism Loop", "desc": "Chronic need to be flawless rooted in early criticism, causing paralysis in growth and stability."\},\
    "self_doubt_cycle": \{"title": "Self-Doubt Cycle", "desc": "Persistent inner critic from early invalidation, blocking decisions and self-expression."\},\
    "avoidance_pattern": \{"title": "Avoidance Pattern", "desc": "Habit of escaping emotions or challenges, often from early overwhelm or trauma."\},\
    "trust_deficit": \{"title": "Trust Deficit", "desc": "Deep difficulty trusting others, stemming from betrayal or inconsistency in formative years."\},\
    "impulsivity_recursion": \{"title": "Impulsivity Recursion", "desc": "Repeated impulsive reactions born from early chaos or lack of containment."\},\
    "emotional_neglect_echo": \{"title": "Emotional Neglect Echo", "desc": "Lingering emptiness and difficulty feeling worthy of care, from childhood emotional absence."\},\
    "failure_phobia": \{"title": "Failure Phobia", "desc": "Terror of failing that paralyzes action, usually from early harsh punishment for mistakes."\}\
\}\
\
# SCORING & RECURSION ENGINE\
...
def detect_recursions(responses, element_scores):\
    ...
    ctq_emotional = sum(["Often true", "Very often true"].count(responses.get(str(i), "")) * 5 + ["Sometimes true"].count(responses.get(str(i), "")) * 3 for i in range(68, 73))\
    ctq_physical = sum(["Often true", "Very often true"].count(responses.get(str(i), "")) * 5 + ["Sometimes true"].count(responses.get(str(i), "")) * 3 for i in range(72, 74))\
    ctq_sexual = sum(["Often true", "Very often true"].count(responses.get(str(i), "")) * 5 + ["Sometimes true"].count(responses.get(str(i), "")) * 3 for i in range(74, 76))\
    ctq_neglect = sum(["Often true", "Very often true"].count(responses.get(str(i), "")) * 5 + ["Sometimes true"].count(responses.get(str(i), "")) * 3 for i in range(76, 78))\
\
    mother_narrative = responses.get("79", "").lower()\
    conflict_narrative = responses.get("80", "").lower()\
    abandon_kw = mother_narrative.count("distant") + conflict_narrative.count("left") + conflict_narrative.count("abandon")\
    doubt_kw = mother_narrative.count("critic") + conflict_narrative.count("doubt") + mother_narrative.count("never good")\
\
    scores = \{\}\
...
\
        if key == "abandonment_fear":\
            score = 30 + ace * 8 + abandon_kw * 6\
            if element_scores.get("Connection", 100) < 65: score += 25; reasons.append("Low Connection")\
            if element_scores.get("Meaning", 100) < 65: score += 20; reasons.append("Low Meaning")\
        elif key == "perfectionism_loop":\
            score = 40 + ctq_emotional * 3\
            if element_scores.get("Growth", 100) < 65: score += 30; reasons.append("Low Growth")\
        elif key == "self_doubt_cycle":\
            score = 35 + doubt_kw * 8 + ctq_emotional * 2\
            if element_scores.get("Growth", 100) < 65: score += 20; reasons.append("Low Growth")\
            if element_scores.get("Meaning", 100) < 65: score += 15; reasons.append("Low Meaning")\
        elif key == "avoidance_pattern":\
            score = 45 + ctq_sexual * 5\
            if element_scores.get("Vitality", 100) < 65: score += 25; reasons.append("Low Vitality")\
        elif key == "trust_deficit":\
            score = 40 + ace * 7 + ctq_emotional * 2\
            if element_scores.get("Connection", 100) < 65: score += 30; reasons.append("Low Connection")\
        elif key == "impulsivity_recursion":\
            score = 40 + ctq_physical * 4\
            if element_scores.get("Stability", 100) < 65: score += 25; reasons.append("Low Stability")\
        elif key == "emotional_neglect_echo":\
            score = 45 + ctq_neglect * 4\
            if element_scores.get("Meaning", 100) < 65: score += 30; reasons.append("Low Meaning")\
            if element_scores.get("Connection", 100) < 65: score += 20; reasons.append("Low Connection")\
        elif key == "failure_phobia":\
            score = 35 + ctq_emotional * 3 + doubt_kw * 6\
            if element_scores.get("Growth", 100) < 65: score += 30; reasons.append("Low Growth")\
            if element_scores.get("Stability", 100) < 65: score += 20; reasons.append("Low Stability")\
\
        score = min(100, score)\
        if score > 35:\
            scores[key] = \{"score": score, "reasons": reasons + [f"ACE \{ace\}/7", f"CTQ-Emo \{ctq_emotional\}"]\}\
\
    candidates = sorted(scores.items(), key=lambda x: x[1]["score"], reverse=True)[:3]\
    result = []\
    for key, data in candidates:\
        affected = [el for el, sc in element_scores.items() if sc < 65]\
        result.append(\{\
            "title": recursions_defs[key]["title"],\
            "description": recursions_defs[key]["desc"],\
            "strength": data["score"],\
            "reasons": data["reasons"],\
            "currently_pulling": affected if affected else ["None \'96 already largely resolved"]\
        \})\
    return result if result else [\{"title": "No major recursion detected", "description": "Your biome is free and expanding.", "strength": 0, "currently_pulling": [], "reasons": []\}]\
\
@app.route('/')\
def index():\
...
def assess():\
    data = request.json\
    element_scores = calculate_element_scores(data)\
    recursions = detect_recursions(data, element_scores)\
    avg_recursion_strength = sum(r["strength"] for r in recursions) / max(1, len(recursions))\
    return jsonify(\{"scores": element_scores, "recursions": recursions, "avg_recursion": avg_recursion_strength\})\
\
# FULL HTML + Three.js Biome + Enhanced Results\
HTML_FULL_MVP = """\
<!DOCTYPE html>\
<html lang="en">\
<head>\
  <meta charset="UTF-8" />\
  <title>LRS v5.0 \'96 Latent Recursion System</title>\
...
\
    <div class="text-center mb-12">\
      ...
      <p class="mt-4 text-gray-400" id="progress-text">Vitality \'96 0/80</p>\
    </div>\
\
...
\
    <div id="biome-container" class="my-16 text-center hidden">\
      ...
      <p class="mt-6 text-lg text-gray-400">The ring tightens when recursions are active \'97 expands when resolved</p>\
    </div>\
\
...
\
  <script>\
    ...
    let responses = \{\}, currentElement = 0;\
...
\
    function initBiome() \{\
      ...
      const rg = new THREE.TorusGeometry(5,0.2,16,100);\
    ...
    function updateBiome(scores, avgRec)\{\
      trees.forEach(t=>scene.remove(t)); trees=[];\
      const config = [\
        \{el:"Vitality",x:-4.5,color:0x00ff88\},\
        \{el:"Connection",x:-2.5,color:0x4488ff\},\
        \{el:"Environment",x:-0.5,color:0xffaa00\},\
        \{el:"Growth",x:1.5,color:0xff00ff\},\
        \{el:"Stability",x:3.5,color:0xffff00\},\
        \{el:"Meaning",x:5.5,color:0xff0088\}\
      ];\
      config.forEach(c=>\{\
        const h = (scores[c.el]||50)/100*5 + 0.8;\
        ...
        tree.position.set(c.x,h/2,0); scene.add(tree); trees.push(tree);\
      \});\
      const constriction = 1 + (avgRec/100)*0.6;\
      ring.scale.set(constriction,constriction,1);\
      ring.material.color.setHex(avgRec>60?0xff0033:avgRec>30?0xff8800:0x00ff88);\
    \}\
\
    function updateProgress()\{\
      const a = Object.keys(responses).length;\
      document.getElementById('progress').style.width = (a/80*100)+'%';\
      document.getElementById('progress-text').innerText = `$\{elements[currentElement]\} \'96 $\{a\}/80`;\
    \}\
\
    function loadElement()\{\
      const el = elements[currentElement];\
      const qs = element_groups[el] || element_groups['Recursion'];\
      const div = document.getElementById('assessment');\
      div.innerHTML = `<h2 class="text-4xl font-bold text-teal-400 mb-10">$\{el\}</h2>`;\
      qs.forEach(q=>\{\
        const block = document.createElement('div');\
        block.className = 'bg-slate-800 p-6 rounded-2xl mb-6';\
        block.innerHTML = `<p class="font-medium text-lg mb-4">$\{q.id\}. $\{q.text\}</p>`;\
        if(q.options.length>0)\{\
          const opts = document.createElement('div');\
          opts.className = 'grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-3';\
        ...
        \}else\{\
          ...
          ta.className = 'w-full p-4 bg-slate-700 rounded text-white';\
          ta.placeholder = 'Share freely (200\'96300 words)';\
          block.appendChild(ta);\
        \}\
        div.appendChild(block);\
      \});\
      const btn = document.createElement('button');\
      btn.textContent = currentElement<6?'Next Element \uc0\u8594 ':'Generate My Biome & Recursions';\
...
\
        ...
        \}else\{\
          d.recursions.forEach((r,i)=>\{\
            const pull = r.currently_pulling.length>0 ? r.currently_pulling.join(", ") : "None \'96 already largely resolved";\
            html += `\
            <div class="p-8 bg-gradient-to-br from-red-950 to-purple-950 rounded-3xl border $\{r.strength>70?'border-red-500':'border-orange-500'\}">\
              ...
              <p class="text-5xl font-bold my-4 $\{r.strength>70?'text-red-400':r.strength>40?'text-orange-400':'text-yellow-400'\}">$\{r.strength\}% Active</p>\
              <p class="text-lg mb-6">$\{r.description\}</p>\
              <p class="text-xl font-semibold text-yellow-300">Currently pulling on: $\{pull\}</p>\
              <p class="text-sm text-gray-400 mt-4">Evidence: $\{r.reasons.join(' \'b7 ')\}</p>\
            </div>`;\
          \});\
        \}\
\
        ...
        <div class="bg-slate-800 p-10 rounded-3xl text-lg space-y-4">\
          <p>1. Acknowledgement \'96 Days 1\'965: Journal daily about repeating patterns</p>\
          <p>2. Analysis \'96 Days 6\'9610: Map causes and cycles</p>\
          <p>3. Self-Awareness \'96 Days 11\'9615: Examine your role honestly</p>\
          <p>4. Acceptance \'96 Days 16\'9620: Radical acceptance of reality</p>\
          <p>5. Resolution \'96 Days 21\'9625: Make clear decisions</p>\
          <p>6. Re-Design \'96 Days 26\'9630: Build new paths</p>\
          <p>7\'968. Action & Tracking \'96 Ongoing daily steps + weekly check-ins</p>\
        </div>`;\
\
